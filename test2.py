...
# =====================
# GridWorld Gym Environment with Stochastic Movement and Two Goals
# =====================
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n=5, num_barriers=2, r_goal=10, r_barrier=-10, seed_nr=0):
        ...
        self.goals = random.sample(possible_goal_positions, 2)  # Now we have two goals

        # Set rewards for both goals
        for goal in self.goals:
            self.rewards[goal[0], goal[1]] = r_goal

        # Place barriers (avoiding agent and both goals)
        possible_barrier_positions = [(i, j) for i in range(n) for j in range(n)
                                      if (i, j) != self.agent_pos and (i, j) not in self.goals]
    ...
    def step(self, action):
        ...
        # Episode ends when reaching either goal
        done = (x, y) in self.goals
...
# =====================
# Policy Iteration Logic (Updated for two goals)
# =====================
...
def policy_evaluation(env, v, pi, gamma, theta):
    ...
    while delta > theta:
        ...
        for x in range(n):
            for y in range(n):
                # Skip goal states (they are terminal)
                if (x, y) in env.goals:
                    continue
                total = 0
...
def policy_improvement(env, v, pi, gamma):
    ...
    for x in range(n):
        for y in range(n):
            # Skip goal states (no action needed)
            if (x, y) in env.goals:
                continue
            old_pi = pi[x, y, :].copy()
            q_values = np.zeros(4)
...
# =====================
# Plotting Functions (Updated for two goals)
# =====================
def save_value_function(env, value_function, filename="value_function.png"):
    ...
    for i in range(env.n):
        for j in range(env.n):
            ...
            elif (i, j) in env.goals:  # Goal (now multiple goals)
                goal_idx = env.goals.index((i, j)) + 1
                plt.gca().add_patch(plt.Rectangle((j-0.5, i-0.5), 1, 1, fill=True, color='green', alpha=0.3))
                plt.text(j, i, f'GOAL {goal_idx}', ha='center', va='center', fontweight='bold', fontsize=8)
    ...
    plt.title('Value Function with Cell Types (Two Goals)')
    plt.xlabel('X')
    plt.ylabel('Y')
...
def save_policy(env, policy, filename="policy.png"):
    ...
    for i in range(env.n):
        for j in range(env.n):
            ...
            if env.rewards[i, j] < -1:  # Barrier
                ...
                plt.text(j, i, f'GOAL {goal_idx}', ha='center', va='center', fontweight='bold',
    ...
    plt.title('Optimal Policy with Cell Types (Two Goals)')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.grid(True, color='black', linestyle='-', linewidth=0.5)
    plt.xticks(range(env.n))
...
if __name__ == "__main__":
    ...
    env = GridWorldEnv(n=8, num_barriers=5, r_goal=100, r_barrier=-100, seed_nr=10)
    ...
    optimal_policy, final_v = policy_iteration(env, gamma=0.9, theta=1e-3)
    
    print("\n" + "=" * 50)
    print("Environment Details:")
    print("=" * 50)
    print("Goal positions:", env.goals)
    ...
    # Save value function
    save_value_function(env, final_v, "output/value_function.png")
    
    # Save policy
    save_policy(env, optimal_policy, "output/policy.png")
    
    # Record simulation video
    simulate_and_record(env, optimal_policy, "output/gridworld_simulation.mp4")
